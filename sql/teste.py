...
# def cruza_shp_planilha(pasta_saida):
#     for arquivo in os.listdir(pasta_saida):
#         # Verifica se o arquivo tem a extensão .shp
#         if arquivo.endswith('.geojson') and arquivo.startswith('FAZENDA'):
#             if arquivo.startswith('FAZENDA'):
#                 print('AQUI EU PRECISO CRUZAR O ARQUIVO GEOJSON "FAZENDA" COM A PLANILHA excel "FAZENDA" que estão na pasta. Preciso que combinem a coluna cod_fazenda da planilha com a planilha fazenda do shp. e passe a coluna geometria do shp para a planilha')

#             if arquivo.startswith('TALHAO'):
#                 print('Oi victoria, voce esta mt brava')
